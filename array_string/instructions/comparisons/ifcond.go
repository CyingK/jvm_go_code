...
func (self *IFEQ) Execute(frame *rtda.Frame) {
	...
	if flag == 0 {
		fmt.Printf("ifeq: 栈顶标识：%d, 等于 0, 程序计数器跳转至: %d\n", flag, self.Offset)
...
func (self *IFNE) Execute(frame *rtda.Frame) {
	flag := frame.GetOperandStack().PopInt()
	if flag != 0 {
		fmt.Printf("ifne: 栈顶标识：%d, 不等于 0, 程序计数器跳转至: %d\n", flag, self.Offset)
		base.Branch(frame, self.Offset)
	}
}
...
func (self *IFLT) Execute(frame *rtda.Frame) {
	flag := frame.GetOperandStack().PopInt()
	if flag < 0 {
		fmt.Printf("iflt: 栈顶标识：%d, 小于 0, 程序计数器跳转至: %d\n", flag, self.Offset)
		base.Branch(frame, self.Offset)
	}
}
...
func (self *IFLE) Execute(frame *rtda.Frame) {
	flag := frame.GetOperandStack().PopInt()
	if flag <= 0 {
		fmt.Printf("ifle: 栈顶标识：%d, 小于等于 0, 程序计数器跳转至: %d\n", flag, self.Offset)
		base.Branch(frame, self.Offset)
	}
}
